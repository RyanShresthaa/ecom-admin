...
type ContactField = "name" | "phone" | "email" | "address" | "message";
...
const Contact: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setFormData((prev) => ({
      ...prev,
      name: prev.name || sanitizeNameInput(user.name || ""),
      email: prev.email || sanitizeEmailInput(user.email || ""),
    }));
    setPrefilled(true);
  ...
  const setFieldError = (name: ContactField, message: string | null) => {
    setFieldErrors((prev) => {
      const next = { ...prev };
  ...
  const sanitizeField = (name: ContactField, value: string): string => {
    switch (name) {
      case "name":
      ...
      case "address":
        return sanitizeSingleLine(value, 200);
      case "message":
        return sanitizeMultiline(value, 2000);
      default:
        return value;
    }
  };

  const validateField = (name: ContactField, value: string): string | null => {
    switch (name) {
      case "name":
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!check.ok) {
      ...
      setStatus("err");
      setStatusMsg(firstFieldError(check.errors) || "Please fix the highlighted fields.");
      setSubmitting(false);
      return;
    }

    try {
      const title = orderRef ? `Order help — ${orderRef}` : `Contact from ${check.value.name}`;
      const comment = [
        check.value.message,
        "",
      ...
      setFieldErrors({});
      setFormData({
        name: isLoggedIn && user ? sanitizeNameInput(user.name || "") : "",
        phone: "",
        email: isLoggedIn && user ? sanitizeEmailInput(user.email || "") : "",
        address: "",
        message: "",
      });
    } catch (err) {
