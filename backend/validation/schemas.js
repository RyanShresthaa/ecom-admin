...
function normalizeAddressCountry(country) {
    const mode = getCachedRegionMode();
    if (mode === 'nepal') {
        const c = String(country || '').trim();
        return c || 'Nepal';
    }
    return normalizeUsCountry(country) || 'United States';
}

function runAddressValidation(data) {
...
/** Shared shipping address body (create / update) — rules follow shop region_mode */
export const addressBodySchema = z
    ...
    .superRefine((data, ctx) => {
        const result = runAddressValidation(data);
        if (!result.ok) {
    ...
    .transform((data) => {
        const result = runAddressValidation(data);
        if (!result.ok) {
            throw new Error(firstAddressError(result));
        }
        return { ...result.value, _id: data._id };
    });

/** PUT /api/address/update — same fields + id */
export const addressUpdateBodySchema = addressBodySchema;
