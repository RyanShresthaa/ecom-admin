 ...
 */
import { createCoupon, findAllCoupons, findCouponByCode, deleteCoupon, updateCoupon } from '../models/coupon.model.js';
...
export async function updateCouponController(req, res) {
    try {
        ...
        const data = await updateCoupon(id, req.body || {});
        if (!data) {
        ...
        return res.json({ message: 'Coupon updated', data, error: false, success: true });
    } catch (e) {
        return res.status(500).json({ message: e.message, error: true, success: false });
    }
}
