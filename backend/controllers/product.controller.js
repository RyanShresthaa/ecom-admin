...
export const getProductController = async (request, response) => {
    try {
        ...
        const page = Math.max(1, Number(src.page) || 1);
        const limit = Math.min(100, Math.max(1, Number(src.limit) || 10));
        ...
        const { data, totalCount } = await findProducts({
            search,
            published,
            skip,
            limit,
...
export const getProductBySlugController = async (request, response) => {
    try {
...
export const deleteProductDetails = async (request, response) => {
    try {
        ...
        return response.json({
            message: result.soft
                ? 'Product unpublished (linked to past orders)'
                : 'Delete successfully',
            data: result,
            error: false,
            success: true,
