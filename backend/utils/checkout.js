...
import { toDisplayAmount, normalizeCurrencyCode } from './currency.js';
...
export async function resolveCheckoutLines({ userId, list_items, couponCode, useCart }) {
    ...
    const displayCurrency = normalizeCurrencyCode(
        settings.currency || (String(settings.region_mode).toLowerCase() === 'nepal' ? 'NPR' : 'USD'),
    );
    ...
    for (const el of rawItems) {
        ...
        const baseUnit = unitPriceAfterDiscount(product.price, product.discount);
        const unitPrice = toDisplayAmount(baseUnit, settings);
        lines.push({
    ...
    // Coupon fixed amounts + shipping are already in shop (display) currency from settings
    ...
    summary.priceBaseCurrency = normalizeCurrencyCode(settings.price_base_currency || 'NPR', 'NPR');
    summary.usdNprRate = Number(settings.usd_npr_rate) || 133;
