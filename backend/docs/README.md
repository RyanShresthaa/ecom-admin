...
| Cookie | Path | Purpose |
|--------|------|---------|
| `accessToken` | `/` | Short-lived JWT (httpOnly); TTL synced with `ACCESS_TOKEN_EXPIRES` |
| `refreshToken` | `/api/user` | Long-lived refresh (httpOnly), rotated on refresh |
| `csrfToken` | `/` | Not httpOnly — copy to header |

**Login / refresh JSON** returns `csrfToken` (and `user` on login) only — **never** access or refresh JWTs. Auth is cookie-based (`credentials: include` / `withCredentials: true`). Optional `Authorization: Bearer <accessJWT>` is still accepted when correctly formatted.
...
| Path should be auth-bootstrap exempt? | `middleware/csrf.js` → `AUTH_BOOTSTRAP_PATHS` |
